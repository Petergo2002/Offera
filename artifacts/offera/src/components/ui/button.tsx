...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default:
          "bg-gradient-to-br from-primary to-primary-dim text-primary-foreground border-none shadow-sm hover:shadow-subtle hover:-translate-y-[1px] active:translate-y-[0px] active:scale-[0.98]",
        destructive:
          "bg-destructive text-destructive-foreground hover:opacity-90 active:scale-[0.98]",
        outline:
          "border border-border/15 bg-transparent hover:bg-secondary/50",
        secondary:
          "bg-secondary text-secondary-foreground hover:bg-secondary/80",
        ghost: 
          "hover:bg-secondary/50 hover:text-foreground text-primary",
        link: 
      ...
      size: {
        ...
        ghost: "min-h-9 px-6 py-2", /* High horizontal padding */
        icon: "h-9 w-9",
      },
    },
    defaultVariants: {
